...
const { JS_DAY_TO_HEBREW, DAYS_OF_WEEK_KEYS, APPOINTMENT_STATUS } = require('../config/constants');
...
/**
 * Filter out dates where the staff already has a non-cancelled appointment at the requested time slot.
 * Rule: Recurring appointment is created at a slot ONLY when:
 *   - there is no appointment at that time, OR
 *   - the only appointment at that time is cancelled (cancelled slots can be re-used).
 * So we only treat booked/scheduled as conflicts; cancelled does not block.
 *
 ...
 * @param {object} prisma - Prisma client
 * @returns {Promise<string[]>} - Dates with no existing booked/scheduled appointment at that time
 */
async function filterRecurringDatesByExistingAppointments(
  ...
  // Only booked/scheduled block the slot; cancelled appointments do not block (slot can be re-used)
  const existingAppointments = await prisma.appointment.findMany({
    where: {
      staffId,
      startDate: { lte: rangeEnd },
      endDate: { gte: rangeStart },
      appointmentStatus: { not: APPOINTMENT_STATUS.CANCELLED },
    },
