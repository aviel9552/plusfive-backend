  ...
  try {
    ...
    for (const user of activeUsers) {
      try {
        ...
        // Get WhatsApp message count for PREVIOUS month (unbilled messages only)
        // Cron runs on 1st of month, so count previous month's messages
        const previousMonth = getPreviousMonthRange();
        const usageCount = await prisma.whatsappMessage.count({
          where: { 
            userId: user.id, 
            createdAt: { 
              gte: previousMonth.start,  // Start of previous month
              lt: previousMonth.end      // Start of current month (exclusive)
            },
        ...
        if (usageCount > 0) {
          try {
            // Report usage to Stripe - this triggers automatic invoice generation
            ...
            console.log(`✅ Reported ${usageCount} messages usage to Stripe for user ${user.email}`);

            // Mark messages as billed in database
            ...
            console.log(`✅ Marked ${usageCount} messages as billed for user ${user.email}`);

            // Send usage report email to user
            await sendUsageReportEmail(user.email, usageCount);
          } catch (error) {
            console.error(`❌ Failed to process usage billing for ${user.email}:`, error.message);
            // Still try to send email even if Stripe reporting fails
            try {
              await sendUsageReportEmail(user.email, usageCount);
            } catch (emailError) {
              console.error(`❌ Failed to send usage email to ${user.email}:`, emailError.message);
            }
          }
        }
...
function getPreviousMonthRange() {
  ...
  // Start of current month (exclusive end for range)
