...
const reportUsageForMonth = async () => {  
  try {
    ...
    if (isTestMode) {
      ...
      console.log(`🧪 Test mode will ONLY apply to subscriptions with 'day' interval`);
      console.log(`🧪 For 'day' interval: Billing period will use CURRENT DATE (today)`);
      console.log(`🧪 For other intervals (week/month/year): Normal billing period will be used\n`);
    }
    ...
    for (const user of activeUsers) {
      try {
        ...
        let billingPeriod;
        // TEST MODE: Only apply for 'day' interval, use current date (today) for billing period
        if (isTestMode && billingInterval.toLowerCase() === 'day') {
          const now = new Date();
          ...
          billingPeriod = {
            ...
            description: 'Today (TEST MODE - Day Interval Only)'
          };
          
          console.log(`🧪 TEST MODE ENABLED for DAY interval - Using current date (today) for billing period`);
        } else if (stripePeriodStart && stripePeriodEnd) {
          // PRODUCTION MODE: Use Stripe's actual billing period (e.g., 15 Nov - 15 Dec)
          // This ensures messages are counted for the exact subscription billing cycle
          billingPeriod = {
          ...
          if (isTestMode && billingInterval.toLowerCase() !== 'day') {
            console.log(`ℹ️  TEST MODE is enabled but interval is '${billingInterval}' - using normal billing period (test mode only works for 'day' interval)`);
          }
        }
        ...
        if (usageCount > 0) {
          try {
            ...
            if (isTestMode && billingInterval.toLowerCase() === 'day') {
              console.log(`🧪 TEST MODE (Day Interval): Payment will be charged for today's messages only`);
            } else {
