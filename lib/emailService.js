...
// IMPORTANT: Only send email if messageCount > 0 (there are actual messages to report)
const sendUsageReportEmail = async (userEmail, messageCount) => {
  try {
    // Validate: Don't send email if no messages
    if (!messageCount || messageCount === 0) {
      console.log(`⚠️  Skipping usage report email for ${userEmail} - No messages to report (messageCount: ${messageCount})`);
      return;
    }

    // Get previous month (since billing is for previous month)
    const now = new Date();
    ...
    console.log(`📧 Sending usage report email to ${userEmail} for ${previousMonth} - ${messageCount} messages`);

    const emailData = {
      to: userEmail,
      subject: `Monthly Usage Report - ${previousMonth}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #333;">Monthly Usage Report</h2>
          <p>Hello,</p>
          <p><strong>WhatsApp Usage: ${messageCount} ${messageCount === 1 ? 'message' : 'messages'} sent in ${previousMonth}.</strong></p>
          <p>Here's your monthly usage report for <strong>${previousMonth}</strong>:</p>
          ...
          <p style="color: #28a745; font-weight: bold; font-size: 16px; margin: 20px 0;">
            ✅ Auto Pay Enabled: Your card will be automatically charged based on message count at the end of this billing cycle.
          </p>
          
          <p><strong>Charges will be applied based on message count, and you will receive a payment receipt.</strong></p>
          
          <p>This process is automated through a cron job, which sends the billing email to you at the end of each month. ✅</p>
          
          <p>This usage has been reported to Stripe and will be automatically charged to your saved payment method. No manual payment required.</p>
          
          <div style="text-align: center; margin: 30px 0;">
            <a href="${process.env.FRONTEND_URL}/app/subscription-and-billing" 
               style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                      color: white; 
                      padding: 12px 30px; 
                      text-decoration: none; 
    ...
    console.log(`✅ Usage report email sent successfully to ${userEmail}`);
  } catch (error) {
