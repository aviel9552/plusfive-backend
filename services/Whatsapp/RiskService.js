...
class RiskService {
  ...
  async sendMessage(phoneNumber, message) {
    try {
      ...
      console.error('Error sending risk service message:', error);
      throw error;
    }
  }
  ...
  async sendClosureMessage(phoneNumber) {
    try {
      ...
      console.error('Error sending closure message:', error);
      throw error;
    }
  }
  ...
  async startAtRiskConversation(customerName, phoneNumber) {
    try {
    ...
    try {
      await this.prisma.conversationState.upsert({
        ...
        update: {
          ...
          status: stateData.status || 'at_risk',
  ...
  async loadConversationFromDatabase(phoneNumber) {
    try {
      ...
      if (conversation && !conversation.conversationEnded && conversation.status === 'at_risk') {
  ...
  clearAllConversationStates() {
    ...
    return { message: 'All conversation states cleared' };
  }
}
